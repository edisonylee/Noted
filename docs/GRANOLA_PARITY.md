...
| Ask across team knowledge | [Chat](https://docs.granola.ai/help-center/getting-more-from-your-notes/chatting-with-your-meetings) | Implemented: selected-meeting, folder, space and workspace scopes; transcript search, evidence links, configured local/hosted/BYOK model; all authorized history participates in ranking | Retrieval uses keyword ranking and bounded excerpts. Multi-turn conversations, attachments and per-chat model choice remain open |
| Private answer history | [Chat](https://docs.granola.ai/help-center/getting-more-from-your-notes/chatting-with-your-meetings) | Implemented: explicitly saved answers visible only to their account, sources rechecked on reopening; invalidated when evidence changes or access is removed | Full automatic conversation history and history pagination beyond the latest 100 remain open |
...
| API and MCP | [Granola API](https://docs.granola.ai/help-center/sharing/integrations/granola-api), [MCP](https://docs.granola.ai/help-center/sharing/integrations/mcp) | Noted already has explicit local agent-access foundations. The new team HTTP API authenticates users; it is not yet an independent read-only integration-key or organizational MCP product. Those must preserve grants and avoid granting administrative sessions to agents. |
...
- Production frontend compilation and Rust command compilation are checked.
- The dedicated browser preview uses the real local HTTP service and synthetic
  meetings. It verifies retrieval and permissioned workflows, while generated
