...
const owner = store.authenticate(setup.token),
  org = setup.org,
  mainSpace = store.spaces(owner, org)[0].id;
const invite = store.invite(owner, org, {
  name: "Taylor Chen",
  role: "member",
});
const member = store.accept(invite.token);
const taylor = store.authenticate(member.token!);
const product = store.saveFolder(owner, org, {
  space_id: mainSpace,
  name: "Product",
  description: "Research, priorities, and the decisions behind our work.",
});
const customers = store.saveFolder(owner, org, {
  space_id: mainSpace,
  name: "Customer conversations",
  description: "What we are learning from the people using Fieldwork.",
});
const restricted = store.createSpace(owner, org, {
  name: "Leadership",
  description: "Planning shared with leadership only.",
  visibility: "restricted",
});
const research = store.saveGroup(owner, org, {
  name: "Product team",
  member_ids: [taylor],
});
store.grant(owner, org, mainSpace, {
  kind: "group",
  id: research.id,
  role: "editor",
});
...
examples.forEach(([title, date, summary, folder], i) =>
  store.publish(i % 2 ? taylor : owner, org, {
    space_id: mainSpace,
    source_key: `sample-${i}`,
    title,
    summary,
    transcript: `[00:12] Alex: Let’s review ${title.toLowerCase()}.\n[00:42] Taylor: We should keep the source conversation available so the team can verify the decision.`,
    occurred_at: `${date}T16:00:00Z`,
    folder_ids: [folder],
  }),
);
store.publish(owner, org, {
  space_id: restricted.id,
  source_key: "sample-restricted",
  title: "Leadership planning",
  summary:
    "## Planning\nReview the next quarter’s hiring plan and runway assumptions with the leadership group.",
...
store.saveRecipe(owner, org, {
  ...
  prompt:
    "Summarize decisions that changed this week. Cite the meetings and distinguish confirmed decisions from open questions.",
  kind: "recipe",
});
store.saveRecipe(owner, org, {
  name: "Find open commitments",
  prompt:
    "List open commitments, their owners, and any stated deadlines. Cite the original conversation. Do not invent dates.",
  kind: "recipe",
});
store.saveRecipe(owner, org, {
  name: "Customer interview",
  prompt:
    "Summarize customer goals, current workflow, friction, exact quotes, and follow-ups. Keep hypotheses separate from customer statements.",
  kind: "template",
});
...
const vite = Bun.spawn(
  ["node_modules/.bin/vite", "--config", "vite.team.config.ts"],
