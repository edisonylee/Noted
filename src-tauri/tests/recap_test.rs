...
fn recap_range_and_storage() {
    ...
    save(
        &mut conn,
        "gym",
        json!({"exercises":[{"name":"squat","weight":245}]}),
        "2026-06-11",
    );
    ...
    db::upsert_recap(
        ...
        "2026-06-09",
        "2026-06-11",
        "you trained twice — bench 185, squat 245",
        2,
...
async fn generates_a_grounded_recap() {
    ...
    save(
        &mut conn,
        "gym",
        json!({"exercises":[{"name":"squat","weight":245,"reps":5}]}),
        "2026-06-11",
    );
    save(
        &mut conn,
        "schedule",
        json!({"blocks":[{"task":"coding","hours":4}]}),
        "2026-06-11",
    );
    ...
    assert!(
        lc.contains("bench") || content.contains("185"),
        "mentions the workout"
    );
    assert!(
        lc.contains("squat") || content.contains("245") || lc.contains("cod"),
