...
async fn embeds_and_retrieves_relevant_note() {
    ...
    let notes = [
        (
            "gym",
            "bench press 185 for 5 reps, chest day felt strong",
            json!({"exercises":[{"name":"bench","weight":185,"reps":5}]}),
        ),
        (
            "meals",
            "had a chicken burrito bowl with extra guac for lunch",
            json!({"meal":"chicken burrito bowl"}),
        ),
        (
            "schedule",
            "spent 3 hours coding the noted app this afternoon",
    ...
    let qv = norm(
        ollama::embed("how much did I bench last week?")
            .await
            .unwrap(),
    );
    ...
    println!(
        "bench query top: {:?} d={}",
        hits[0].category, hits[0].distance
    );
    ...
    let ctx: String = hits
        ...
        .map(|(i, h)| {
            format!(
                "[{}] {} on {}: {}\n",
                i + 1,
                h.category.clone().unwrap_or_default(),
                h.event_date,
                h.raw_text
            )
        })
    ...
    assert!(
        answer.contains("185"),
        "answer should cite the 185 lb bench: {answer}"
    );
    ...
    println!(
        "food query top: {:?} d={}",
        hits2[0].category, hits2[0].distance
    );
