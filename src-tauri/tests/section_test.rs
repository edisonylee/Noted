...
fn splits_diary_into_tagged_sections() {
    ...
    assert!(
        hints.contains(&Some("schedule")),
        "schedule routed: {hints:?}"
    );
    ...
    let food = segs
        .iter()
        .find(|s| s.hint.as_deref() == Some("food"))
        .unwrap();
    ...
    let gym = segs
        .iter()
        .find(|s| s.hint.as_deref() == Some("gym"))
        .unwrap();
...
fn stoplist_words_are_not_headers() {
    ...
    assert!(
        segs.iter().all(|s| s.hint.is_none()),
        "stoplisted labels stay untagged"
    );
}
...
fn mid_line_dash_is_not_a_header() {
    ...
    assert!(
        !hints.contains(&Some("chipotle bowl")),
        "mid-line dash is not a header: {hints:?}"
    );
    let food = segs
        .iter()
        .find(|s| s.hint.as_deref() == Some("food"))
        .unwrap();
    assert!(
        food.body.contains("chipotle bowl"),
        "dash line stays in food body: {}",
        food.body
    );
}
...
fn multi_topic_gate() {
    ...
    assert!(!is_multi_topic_candidate(
    ...
    assert!(
        is_multi_topic_candidate(dump),
        "a long multi-sentence dump should segment"
    );
}
