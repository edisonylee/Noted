...
export function PublishMeeting({
  ...
  const [chosenSummary, setChosenSummary] = useState(
    summaryId ?? meeting.summaries[0]?.id ?? null,
  );
  ...
  const destination = data?.spaces.find((s) => s.id === space),
    summary = meeting.summaries.find((s) => s.id === chosenSummary);
  const reviewedTranscript = transcript
    ? meeting.segments
        .map((s) => {
    ...
    : "";
  const publish = async () => {
    if (!org || !space || !summary || busy) return;
    setBusy(true);
    setError("");
    try {
      ...
      if (!installation) {
        ...
        localStorage.setItem("noted-team-publication-origin", installation);
      }
      await api.teamPublishMeeting({
        org,
        id: meeting.id,
        spaceId: space,
        folderIds: folders,
        ...
        sourceKey: `${installation}:${meeting.id}`,
        reviewedContent: {
          title: meeting.title,
  ...
  return (
    <TeamDialog
      title={done ? "Meeting shared" : "Publish a meeting to your team"}
      onClose={onClose}
      busy={busy}
    >
      {done ? (
        ...
        <div className="team-form">
          <p>
            Share a copy of <strong>{meeting.title}</strong>. Review the summary
            before publishing.
          </p>
          <label>
            Team
          ...
          <label>
            ...
            <select
              value={chosenSummary ?? ""}
              onChange={(e) => setChosenSummary(Number(e.target.value))}
              disabled={busy}
            >
          ...
          <div className="team-publication-preview">
            {summary ? (
              <MdBlock md={summary.content_md} />
            ) : (
              <p>Generate a summary before publishing.</p>
            )}
          </div>
          <label className="team-checkbox">
            ...
            />
            Include the full speaker-labeled transcript (
            {meeting.segments.length} segments)
          </label>
          {transcript && (
            <details className="team-transcript">
              <summary>Review transcript</summary>
              <pre>{reviewedTranscript}</pre>
            </details>
          )}
          <p className="team-muted">
            My Notes, audio, and video are not included. Summaries can contain
            information from My Notes, so review the preview. Publishing creates
            a shared copy; future local edits won’t update it automatically.
          </p>
          {error && (
